...
Guidelines for AI Coding Assistants (Gemini / Antigravity / Claude Code) across all projects.
...
- **Never** include `Co-Authored-By: Claude`, `Co-Authored-By: Gemini`, `Co-Authored-By: Antigravity`, or any other AI co-author line in commit messages.
- **Never** append `🤖 Generated with [Claude Code]`, `🤖 Generated by Gemini`, `Created by Antigravity`, or any variant to PR bodies, PR titles, issue bodies, comments, or commit messages.
- **Never** add robot emojis, "Generated with", "Created by AI", "with help from AI", or similar footers/badges to anything written into a repo or GitHub (commits, PRs, issues, code comments, changelogs, docs, READMEs).
...
**Never refer to anything by a bare number or ID.** Not a section number, backlog item, slice, work package, PR, migration, or ticket key standing on its own. Say what the thing IS in plain words, then the number last in parentheses as a pointer. The number is never the name of the thing. Applies everywhere: chat, plans, options inside questions, tables, commit messages, PR bodies.
...
- When providing text meant to be copied verbatim (email drafts, message templates, letters, snippets), do **not** wrap it in a markdown blockquote (`>`).
...
- **Never hard-wrap copy-paste text.** One paragraph = one continuous line, no manual line breaks inside a paragraph. Blank lines between paragraphs are fine; let the terminal soft-wrap the rest.

**Never describe your own working method in a document you write for me.**

- No "I checked X, then Y", no "this is deliberately short", no references to rounds, agents, drafts, or how the text came about.
- The reader wants the content, not a report on how it was produced.
